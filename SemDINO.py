...
from backbone.efficientnet import efficientnet_b3_backbone
...
class Encoder(nn.Module):
    def __init__(self, dim, weights_path=None, efficientnet_weights_path=None,
                use_checkpoint=False):
        super().__init__()
        # NOTE on offline clusters (no internet on compute nodes):
        # pretrained=True here never downloads anything -- backbone/efficientnet.py
        # only loads from a local .pth (efficientnet_weights_path / the
        # EFFICIENTNET_B3_WEIGHTS env var / $TORCH_HOME cache) and falls back to
        # random init with a warning if none is found. See that file's docstring
        # for how to stage the official ImageNet weights onto the HPC.
        self.backbone = efficientnet_b3_backbone(pretrained=True,
        ...
        self.dino = DINOV3Wrapper(weights_path=weights_path)
...
class SemDINO(nn.Module):
    def __init__(self, num_classes=6, dim=128, weights_path=None,
                efficientnet_weights_path=None, use_checkpoint=False):
        super().__init__()
        self.encoder = Encoder(dim, weights_path=weights_path,
                               efficientnet_weights_path=efficientnet_weights_path,
                               use_checkpoint=use_checkpoint)
